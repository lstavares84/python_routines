...
capitais = [
    'SE', 'PA', 'MG', 'RR', 'DF', 'MS', 'MT', 'PR', 'SC', 'CE', 'GO', 'PB',
    'AP', 'AL', 'AM', 'RN', 'TO', 'RS', 'RO', 'PE', 'AC', 'RJ', 'BA', 'MA',
    'SP', 'PI', 'ES'
]

def dijkstra(start, end, distancias, tabela_preco_diesel):
    # Verifique se o nó de início e o nó de destino estão presentes na tabela de distâncias
    if start not in distancias or end not in distancias:
        print("Nó de início ou destino não encontrado na tabela de distâncias.")
        return None, None
    ...
    # Variáveis para armazenar os caminhos com a menor distância e menor custo
    menor_distancia_caminho = None
    menor_custo_caminho = None

    # Variáveis para armazenar a menor distância total e menor custo total
    ...
    while open_set:  # Enquanto houver caminhos não testados...
        ...
        # Verifica se o nó atual não é None
        if current is None:
            break
        ...
        if current == end:
            ...
            # Calcula o custo total percorrido
            total_cost = 0

            # Calcula e imprime as distâncias entre os nós ao longo do caminho, considerando os valores da tabela de preços do diesel
            diesel_costs = []
            for i in range(len(path) - 1):
                node1 = path[i]
                node2 = path[i + 1]
                distance = distancias[node1][node2]
                diesel_cost = distance * tabela_preco_diesel[node1]
                total_cost += diesel_cost  # Adiciona o custo ao custo total
                diesel_costs.append((f'Distância de {node1} para {node2}: {distance}', f'Custo de diesel: {diesel_cost}'))

            # Verifica se este caminho tem a menor distância total
            if total_distance < menor_distancia_total:
                menor_distancia_total = total_distance
                menor_distancia_caminho = path.copy()

            # Verifica se este caminho tem o menor custo total
            if total_cost < menor_custo_total:
                menor_custo_total = total_cost
    ...
    # Imprime o caminho com a menor distância total e o menor custo total
    print('\n\n############## SOLUÇÃO ##############')
    print(f'O caminho mais curto de {start} para {end} com a menor distância é:')
    print(f'Caminho: {menor_distancia_caminho}')
    print(f'Distância total percorrida: {menor_distancia_total}\n')

    print(f'O caminho mais curto de {start} para {end} com o menor custo é:')
    print(f'Caminho: {menor_custo_caminho}')
    print(f'Custo total percorrido: {menor_custo_total}\n')

    return menor_distancia_caminho, menor_custo_caminho

# Para cada combinação de start e end
for start in capitais:
    for end in capitais:
        if start != end:  # Evitar combinações iguais de start e end
